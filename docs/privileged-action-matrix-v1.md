...
| Manage event staff | Add/remove event roles | Admin UI service calls under RLS | Event admin, org admin, or qME superadmin | No uniform audit yet | `event_staff_assignments` RLS enabled and scoped | Product role model is still evolving. Follow-up: station role finalization and clearer UI visibility. |
...
| Manage admin principals/users | Create named admin principal/Auth user | Admin Users surface and server route for Auth creation | qME superadmin for broad principal creation; org/event assignment through scoped tools | Partial operational trace; no uniform audit matrix yet | `admin_principals` RLS enabled; service-role Auth creation server-side | Accept for pilot. Temporary password/invitation/reset UX remains future. |
