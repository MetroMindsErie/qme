...
- Live blocker diagnosis/fix: the walk-up self-registration save was failing on the first create operation, `create_event_check_in_for_guest`, before Auto completion. Live Supabase RPC diagnostics against a no-valid-event payload showed the 7-parameter call returned HTTP 300 because production has both the legacy and current overloaded RPC signatures; sending the current `p_needs_help` argument selects the intended current overload, then reaches normal function execution. Updated `checkInService.createEventCheckIn` to call the current overload explicitly and retain legacy fallback only if that overload is missing.
- UX refinements completed: imported lookup placeholder now says `First name, last name, or email`; phone is labeled as optional recovery contact and explicitly not used for imported-list search; the fallback panel explains that the guest can register and check in now; required self-registration email now has a local Confirm email field with match validation; generic save failure copy now says `Please see the event team.`
- Added `supabase-checkin-imported-registration-email-search.sql` for the required database-side imported-registration email search refinement. This SQL must be applied to production before live acceptance item 2, imported guest search by email, can pass; it does not alter tables or require a data migration.
- Validation after live-acceptance fix/refinement: `npx tsc -b` passed; focused `npx vitest run src\test\checkInService.test.ts src\test\guestEventCheckIn.test.tsx src\test\eventConfig.test.ts` passed; full `npx vitest run` passed with 119 tests; `npx vite build --outDir ..\tmp\vite-build-check --emptyOutDir` passed. New Check-In configuration behavior, imported-registration claim/reconnect paths, and SOTC hook-order fix were preserved.
