...
| Manage event staff | Add/remove limited staff access | Admin UI plus server route for first-time staff Auth creation | Event admin, org admin, or qME superadmin | Partial operational trace; no uniform audit matrix yet | `event_staff_assignments` RLS enabled and scoped; new Auth users are created server-side using service role after caller event/org authority is checked | Accept for pilot. The Staff tab creates only limited staff, not event-admin/station-provider roles. Temporary passwords are stored in `admin_principals.metadata` only until first-login profile completion; replace with invite/reset-password flow. |
